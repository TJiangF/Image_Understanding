...
%         hold off;  
        ...
        check_tmp=tracked_corners(3,:,i-1);
        check_tmp(find(check_tmp(:)~=0))=1;
        checknum=sum(check_tmp);
        if(checknum<18)
            corners_tmp=corner_detector(imgorg(:,:,i));
            [B,IX] = sort(corners_tmp(:),'descend');
            [I,J] = ind2sub(size(corners_tmp), IX);
...
%             outputname=strcat('track_corner',num2str(filenum(i)),'_rubix_m=_',num2str(m),'.jpg');
