...
inx_tool(find(simi_1st(:)>(test_ratio*simi_2nd(:))))=10000;
simi_matched(:)=simi_1st.*inx_tool;
[simi_intensity,Index_match]=sort(simi_matched(:),'ascend');

simi_matched((find(simi_matched(:)<10000000)))=1;
simi_matched((find(simi_matched(:)>=10000000)))=0;
pair_num_match=sum(simi_matched);
pair_num=floor(lowesratio*pair_num_match);
